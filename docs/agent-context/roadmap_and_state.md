...
Refresh 2026-09-25 (CDP vs backend :3000 + Vite :3090, `/tmp/opencode/cdp-m1-refresh.js` uncommitted, read-only): still boot-only — and quieter than before. Zero page exceptions; console holds only the handled Axios 404/405 probe rejections; 47 boot API hits (mount + retry storms on banner/files/favorites/speech-config/files-config/balance, then silence); idle 90s at `/c/new` → literally 0 API hits. No code change; user DB untouched; servers stopped after. The call itself (accept vs suppress) is still open.
Brick 1 DONE 2026-09-25 (user: "each probe is useless CPU/network, disable them?"): 7 quiet stubs in `system.rs` (`/api/banner`→null, `/api/files`→[], `/api/files/config`→{}, `/api/keys`→`{expiresAt:""}`, `/api/agents/tools/:id/auth`→`{authenticated:false}`, `/api/agents/chat/active` GET→[], `/api/balance`→zero credits) — each payload mirrors what its vendored consumer treats as absent/empty (verified by reading the hooks), so UI renders identically with no vendored divergence; the bearer gate covers them by prefix automatically. Locked by `quiet_stubs_keep_removed_surfaces_hidden` (api 54/54 green), fmt + clippy clean. Browser proof (CDP, `/tmp/opencode/cdp-probe-proof.js` uncommitted, read-only GETs): boot 404/405s drop to only the 3 deferred surfaces (favorites/pinned-order/speech-config, 12 hits, 6 handled rejections); composer + full rail render, no banner/attach/mic/balance UI, zero uncaught. Honest note: the saved resources are trivial (loopback 404s) — the win is console cleanliness, not CPU. Brick 2b DONE 2026-09-25 (half): `GET /api/user/settings/favorites` → `[]`. Pre-verified by reading: the `useFavorites` effect only writes the tab-local atom when data arrives, so `[]` renders what the 404 renders (nothing); writes still 404, unchanged. Locked by router test (api 56/56 green), fmt + clippy clean. Browser proof: favorites gone from the boot 404 list; composer + 4-rail + no-banner/attach/mic/balance parity, zero uncaught. Other half DELIBERATELY NOT STUBBED: `GET /api/user/settings/pinned-order` stays 404 — PinnedSection treats a successful fetch as authoritative, so `[]` would enable the drag/Alt+Arrow reorder path whose POST has no backend (worse than today's gated 404). Silencing it properly is a persistence brick (settings store + GET/POST), out of quiet-stub scope — say the word if you want it. Remaining boot 404s: pinned-order ×4 only. User DB untouched; servers stopped after.
Persistence brick DONE 2026-09-25 (user: "ok go"): `user_settings` blob table (migration) + `settings_repo` get/set + `settings_service` pinned-order read-your-writes (`[]` when missing/corrupt, 1000-entry cap with 400 beyond) + `settings.rs` GET/POST (`{"pinnedOrder": string[]}`, precise 400s on wrong shapes). Locked by 4 service tests + router round-trip/400 test (workspace 164/164 green), fmt + clippy clean. Live proof: scratch round-trip POST→GET `["b","a"]`, user-DB GET `[]` (no write); browser boot census shows ZERO 404/405s, ZERO console errors, full UI parity (composer + 4-rail, no removed surfaces, zero uncaught). Pre-fix fail note: pinned-order curled 404 earlier the same day, so the 200-asserting tests fail without the routes; the corrupt-row test fails on a naive `?`-propagating implementation by construction. Scratch destroyed; user DB untouched apart from the additive migration table (applies on next normal start anyway); servers stopped after.
Brick 3 DONE 2026-09-25: `GET /api/agents/tools/calls` → `[]` (found as a 404 on every convo open, missing from the M1 inventory). Pre-verified by reading: `mapToolCalls([])` → `{}` with every lookup missing (same as today's undefined context), the lookup key is `''` without the revoked RUN_CODE grant so nothing can ever match, and tutor ToolActivity reads live SSE frames, not this query. Locked by router test (api 57/57 green), fmt + clippy clean. Browser proof (CDP, read-only open of a user convo): messages render, tools/calls → 200 with no rejection, only pinned-order 404s remain. User DB untouched; servers stopped after.
Brick 2a DONE 2026-09-25: `GET /api/files/speech/config/get` → `{message:"not_found"}` (the client's own "no server defaults" convention — defaults loop skipped, no engine touched). Pre-verified by reading: all three `speechSettingsInitialized` consumers (`AudioRecorder`, `AutoPlayAudio`, `MessageAudio`) sit behind `speechDisabled` render gates, so the false→true flip is UI-inert. Locked by router test (api 55/55 green), fmt + clippy clean. Browser parity proof (CDP, `/tmp/opencode/cdp-speech-proof.js` uncommitted, read-only GETs on a user convo): composer with no mic, settings with no SPEECH tab, "speech" settings-search clean, open convo renders 2 messages with zero read-aloud buttons; speech/config absent from the 404 list. Remaining 404s: favorites/pinned-order (brick 2b) plus a NEW find vs the M1 inventory — `GET /api/agents/tools/calls?conversationId=` 404s when a convo is open (persisted tool-call history for the activity view; live SSE frames unaffected, history reload may lack activity items — follow-up, not this brick). User DB untouched (nothing created); servers stopped after._
