    ...
    (
      ...
      sse.addEventListener('message', async (e: MessageEvent) => {
        try {
          ...
          if (data.concept_annotations != null) {
            // Knowledgeable M7: stash turn annotations for the markdown
            // renderer. Malformed frames are dropped by the helper and never
            // disturb the stream.
            handleConceptAnnotationsEvent(data);
            return;
          }

          if (data.tool_progress != null) {
