...
export default function useSSE(
  ...
  useEffect(() => {
    ...
    sse.addEventListener('message', (e: MessageEvent) => {
      ...
      } else if (data.concept_annotations != null) {
        // Knowledgeable M7: stash turn annotations for the markdown renderer.
        // Unknown to every other handler; malformed frames are dropped by the
        // helper and never disturb the stream.
        handleConceptAnnotationsEvent(data);
      } else if (data.tool_progress != null) {
        // Knowledgeable M4/T14: tutor activity indicator (same guarantees).
        handleToolProgressEvent(data);
