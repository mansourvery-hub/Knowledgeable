...
import ConceptHighlight from '~/knowledgeable/components/ConceptHighlight';
import { remarkConceptHighlight } from '~/knowledgeable/plugins/remarkConceptHighlight';
import type { ConceptAnnotation } from '~/knowledgeable/types';
...
export const getRemarkPlugins = (
  ...
  unicodeCitation,
  mcpUIResourcePlugin,
  // Knowledgeable M7: runs last so it only sees final narrative text nodes.
  // With no annotations (streaming) it leaves the tree untouched.
  [remarkConceptHighlight, { annotations }],
];
