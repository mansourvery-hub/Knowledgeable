...
| T19 | Keyed browser session (real Gemini turn → tools → weak badge + activity) | - | COMPLETE (6/6 CDP checks; retired-model default fixed to gemini-2.5-flash-lite). NOTE 2026-09-23: code defaults still read `gemini-2.5-flash-lite` (3 sites) but `.env.example` now advertises `gemini-3.1-flash-lite` — mismatch unresolved, needs a live keyed check to confirm which model is current. |
...
- Beta prep: commit hygiene DONE 2026-09-23 (tree clean; annotation fix plan landed as PRs #1–3, CI rot fixed as PR #4, all merged). Remaining: keyed structured-generation proof (wiki gen vs live Gemini). Ollama dropped 2026-09-23 (not implementing; generic `with_base_url` retained for any future local relay).
